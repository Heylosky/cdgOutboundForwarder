...
func Publish(sentChan <-chan models.MbRc, qName string) {
	conn, err := amqp.Dial("amqp://admin:csd@123@172.25.240.10:31656/")
	failOnError(err, "Failed to connect to RabbitMQ")
	defer conn.Close()
	...
	failOnError(err, "Failed to declare a queue: "+qName)

	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	for sms := range sentChan {
		...
		failOnError(err, "Failed to publish a message to [Q]outboundSent")
